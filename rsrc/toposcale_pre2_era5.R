...
step=1 					#time step of accumulated fields
...
dat = ncvar_get( nc,'z')
datInterp =aperm(dat, c(4,1,2,3)) # reorder dimensions to fit with rest of code (origin toposcale_pre2)
save(datInterp, file=paste(outRootPl, '/gPl', sep=''))

#INTERPOLATE TAIR
nc=nc_open(infilePLEVEL)
dat = ncvar_get( nc,'t')
datInterp =aperm(dat, c(4,1,2,3))
...
#=======================================================================================================
#			LWIN
# accumulated fields in ERA5 are just accumulated over the step eg 1 hour step = W/m2/h to convert to w/m2 need # to divide by 3600
#=======================================================================================================
nc=nc_open(infileSURF)
...
for(i in coordMap$cells)
	...
	x=coordMap$xlab[i]
	y=coordMap$ylab[i]
	a = indat[x,y,]/ (step*3600)
	lwgridAv=cbind(lwgridAv,a)
	}
