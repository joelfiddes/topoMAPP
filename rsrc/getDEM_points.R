...
pointsFile=args[4]
loncol=as.numeric( args[5] )
latcol=as.numeric( args[6] )
#points input
...
# if (runtype == "points"){
	...
	# check latitudes
	newgridLat=c()
	for (i in 1:length(df[,latcol])){
	# check if longitude is below lower buffer
		if (revtrunc(df[i,latcol]) < lowbuffer) {
		lat=floor(df[i,latcol]) -1
		lon = df[i,loncol]
		newcoords=data.frame(lon,lat)
		newgrid=rbind(newgrid, newcoords)
		}
	# check if longitude is above upper buffer
		if (revtrunc(df[i,latcol]) > upperbuffer) {
		lat=floor(df[i,latcol]) +1
		lon = df[i,loncol]
		newcoords=data.frame(lon,lat)
		newgrid=rbind(newgrid, newcoords)
