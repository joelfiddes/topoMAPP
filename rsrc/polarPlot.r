...
lp<- read.table('~/data/transformedData/IMIS_meta.txt' ,sep=",", header=T)
lp1<- read.table('/home/joel/data/PERMOS/sites_completeMeta.txt' ,sep=",", header=T)
lp2<- read.table('~/data/ibutton/iB_meta.txt' ,sep=",", header=T)
ele<-c(lp1$ele, lp2$ele,lp$ele)
slp=c(lp1$slp, lp2$slp, lp$slp)
asp=c(lp1$asp, lp2$asp, lp$asp)
