	...
	for year in range(len(dateList)-1):
		...
		for grid in initgrids:
			...
			if os.path.isfile(fname1) == False or os.path.isfile(fname2) == False:
				...
				cmd = ["Rscript",  "./rsrc/PBSpixel_merge_100.R" , ensWd , wd , grid , nens , Nclust , sdThresh , R , cores, DSTART , DEND, str(year), str(start1), str(end1), config["main"]["startDate"], config["main"]["endDate"], valShp, valMode]
				subprocess.check_output(cmd)
			else:
